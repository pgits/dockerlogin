...
function get_graffiti_gps_4_json($sqlInstance, $city_Id, $country_code, $application_type, $gender, $age_range, $lat, $long, $radius, $milesOrKilos, $viewer_Id){
	$whereClause = "";
...
*/
	...
	if($sqlInstance != NULL) {
		...
		if($application_type == 9){
			...
			$whereClause = sprintf(" WHERE `envelope_Id` = '%d' AND `country_code` LIKE '%s' AND `expires` > NOW()  ", 
