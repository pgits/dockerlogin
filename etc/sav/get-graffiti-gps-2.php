...
function get_graffiti_gps_2($sqlInstance, $city_Id, $country_code, $application_type, $gender, $age_range, $lat, $long, $radius, $milesOrKilos, $viewer_Id){
	$whereClause = "";
	...
	if($sqlInstance != NULL) {
		...
		if($country_code === $testIfStar || $country_code === $justStar)
			...
			*/
			...
			$whereClause = sprintf(" WHERE `envelope_Id` = '%d' AND `country_code` LIKE '%s' ", $application_type, $country_code); 
...
*/
		if( $sqlInstance->getGraffitiMessages_2($arrayResults, $whereClause, $arrayFilterResults)){
