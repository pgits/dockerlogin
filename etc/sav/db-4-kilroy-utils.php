...
class geoCode {
	...
	var $limitReached;
	var $users_expired;
	var $tally_of_quota_per_user;
	var $MAX_USERS;

        function reverseGeoCode($latt, $longi){
		$this->latitude        	= $latt;
		$this->longitude       	= $longi;
		$users_loop_count	= 0;
		do {
			echo "current user index = [" . $this->current_user_index . "]\n";
			$this->current_user	       = $this->userArray[$this->current_user_index];
			echo "current_user = [" . $this->current_user . "]\n";
			$tally_of_quota_per_user += 4;
                	$this->url1 = "http://api.geonames.org/findNearbyPlaceName?lat=" . $latt . "&lng=" . $longi . "&style=FULL&username=" . $this->current_user;//service_geekgaps";
                	echo $this->url1 . "\n";
                	$this->xml = file_get_contents($this->url1);
                	echo "returned [" . $this->xml . "]\n";
			if (strpos($this->xml,'the hourly limit of 2000 credits') !== false) {
				echo "limit " . $this->tally_of_quota_per_user . " reached toggling user\n";
				$this->tally_of_quota_per_user = 0;//reset it here
				$this->current_user_index = $this->current_user_index + 1;
				$this->users_count = $this->users_count + 1;
				if($this->current_user_index > $this->MAX_USERS){
					$this->current_user_index = 0;
					echo " resetting current user index = 0\n";
				}
				echo "current_user_index now = " . $this->current_user_index . " \n";
				$this->current_user = $this->userArray[$this->current_user_index];
				echo "user changed now = [" . $this->current_user . "]\n";
				if($this->users_loop_count > $this->MAX_USERS){
					echo "we have looped and exhausted our quotas, exiting now\n";
					$this->users_expired = true;
					exit;
				}
			}else if(strpos($this->xml, 'invalid username') !== false) {
				echo "invalid username returned, exiting now [" . $this->current_user . "]\n";
				exit;
			}
			else
    				break;//this should break out of the while loop
		}while ($this->users_expired == false);
                $this->GeoNameId              = $this->value_in('geonameId',$this->xml, true);
                $this->CityName               = $this->value_in('name',$this->xml, true);
                $this->CountryCode            = $this->value_in('countryCode', $this->xml, true);
                $this->CountryName            = $this->value_in('countryName', $this->xml, true);
                $this->StateProvinceName      = $this->value_in('adminName1', $this->xml, true);
                $this->StateProvinceShortNameBeforeKeyValue = $this->value_in('adminCode1', $this->xml, true);
                echo "preKeyValue = [" . $this->StateProvinceShortNameBeforeKeyValue . "]\n";
                $this->StateProvinceShortNameOtherKey = $this->value_in('adminCode1', $this->xml, false);
		...
		$this->current_user_index = 0;
		$this->MAX_USERS          = 2;
		$this->tally_of_quota_per_user = 0;
		$this->current_user 	= NULL;
		$this->userArray 	= array("pgits", "service_geekgaps");
		$this->limitReached 	= false;
		$this->users_expired 	= false;
        }
...
class SqlUtils {
	...
	public function __construct($hostname, $user, $pass, $database){
		...
		$this->sqlCreateCouponDetails = "INSERT INTO " . $database . "." . $this->couponDetailsTable . "(`graffiti_Id`, `city_Id`, `business_Id`, `business_name`, `coupon_url`, `swiggen_desc`, `expires`, `coupon_limit`, `latitude`, `longitude`, `nearest_store_Id`, `walking`) VALUES ( " ;
		$this->sqlCreateCouponDetails2 = "INSERT INTO " . $database . "." . $this->couponDetailsTable . "(`graffiti_Id`, `city_Id`, `business_Id`, `business_name`, `coupon_url`, `swiggen_desc`, `expires`, `coupon_limit`, `latitude`, `longitude`, `nearest_store_Id`, `walking`, `version_flag`) VALUES ( " ;
		...
		$this->sqlScriptAddCityId2 = "INSERT INTO " . $database . "." . $this->citiesTable . "( `CityName` , `State` , `Country`, `CountryCode`, `first_latitude`, `first_longitude`, `GeoNameId`, `Version`) VALUES (";
		...
		$this->sqlCouponDetailsUpdateStr = "UPDATE " .  "`" . $database . "`" .  "." .  $this->couponDetailsTable . " SET version_flag = "; //where author_alias  = 
...
//UPDATE `produdn8_Graffiti`.`whereAmI` SET `x_location` = '20', `y_location` = '40', `last_mobile_request_time` = '', `last_mse_refresh_time` = NOW() WHERE `whereAmI`.`macAddress` = '00:00:00:00:00:01';
	...
	public function postGraffitiMessage3($sqlInstance2, $author_Id, $icon_name, $message, $city_Id, $country_code, $envelope_Id, $x, $y, $z, $away, $date, $until){
		$bInsert = false;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("sqlPostGraffitiMessage begin = " . $this->sqlPostGraffitiMessage2);
		//$strSqlStep1 = sprintf(" '%s', '%s', '%s'", $username, $message, $country_code);
		...
		$strSqlStep1 = sprintf("'%s', '%s', '%s', '%s', '%s', '%d', '%f', '%f', '%f', '%d', '%s', '%s'", $author_Id, $icon_name, $message, $city_Id, $country_code, $envelope_Id, $x, $y, $z, $away, $date, $until);
		//$strSql = $this->sqlPostGraffitiMessage2 . $strSqlStep1 . $strSqlStep2 . $strSqlStep3 ");";
		$strSql = $this->sqlPostGraffitiMessage2 . $strSqlStep1 . ");";
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("sqlPostGraffitiMessage.strSql = [" . $strSql . "]\n");
		$res = mysqli_query($sqlInstance2->mysqli, $strSql);
		if($res){
			$bInsert = mysqli_insert_id($sqlInstance2->mysqli);
		}
		return $bInsert;
	}
	...
	public function getGraffitiMessages2 (&$arrayResults, $selectStatement){
		$bFound = false;
        	$sqlString = $selectStatement;
		$increment = 0;
		//if($DEBUG_DATABASE_FLAG == TRUE)
		   echo("sqlGetGraffitiMessages2 = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			while($resultsArrayFetch = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				if(count($resultsArrayFetch) > 0) 
				{
					...
					$productRowResult->version_right_of_period= $resultsArrayFetch['version_right_of_period'];
					$bFound = true;
					$arrayResults = $productRowResult;
					//echo("getGraffitiMessages2:found graffitiMessageInstance now = [" . $arrayResults . "]\n");
					return true;
				} else {
					echo("failed to locate graffitiMessages\n");
					return false;
	...
	public function getStoreDetails2 ($sqlInstance2, &$productRowResult, $whereClause){
		$bFound = false;
		//printf("sqlGetStoreDetails = [%s]\n", $this->sqlGetStoreDetails);
        	$sqlString = $sqlInstance2->sqlGetStoreDetails . $whereClause;
		$increment = 0;
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlGetStoreDetails = [" . $sqlString . "]\n");
        	$res = mysqli_query($sqlInstance2->mysqli, $sqlString);
		if($res){
			while($resultsArrayFetch = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				if(count($resultsArrayFetch) > 0) 
				{
					$productRowResult->store_Id    		= $resultsArrayFetch['store_Id'];
/*
					$store_name1    			= $resultsArrayFetch['store_name'];
					echo "store name now = [" . $store_name1 . "]\n";
             				$store_name2 				= str_replace("'", "\\'", $store_name1);
             				$productRowResult->store_name 		= str_replace("&", "+", $store_name2);
*/
             				$productRowResult->store_name 		= $resultsArrayFetch['store_name'];
             				$productRowResult->store_address1 = $resultsArrayFetch['store_address1'];
/*
					echo "store name now = [" . $productRowResult->store_name . "]\n";
					$store_address1 	= $resultsArrayFetch['store_address1'];
             				$store_address1_a = str_replace("'", "\\'", $store_address1);
             				$productRowResult->store_address1 = str_replace("&", "+", $store_address1_a);
*/
             				$productRowResult->store_address2 = $resultsArrayFetch['store_address2'];
/*
					$store_address2 	= $resultsArrayFetch['store_address2'];
             				$store_address2_a = str_replace("'", "\\'", $store_address2);
             				$productRowResult->store_address2 = str_replace("&", "+", $store_address2_a);
*/
					$productRowResult->city 		= $resultsArrayFetch['city'];
					$productRowResult->state_province 	= $resultsArrayFetch['state_province'];
					$productRowResult->zip_code 		= $resultsArrayFetch['zip_code'];
					...
					$productRowResult->website_url 		= $resultsArrayFetch['website_url'];
					$productRowResult->business_Id 		= $resultsArrayFetch['business_Id'];
					$productRowResult->branch_Id 		= $resultsArrayFetch['branch_Id'];
					return true;
				} else {
					echo("failed to locate storeDetails\n");
					return false;
	...
	public function createStoreDetails ($storeDetails){
		$bInsert = -1;
		$bFound = false;
		//$this->sqlCreateStoreDetails = "INSERT INTO " . $database . "." . $this->storeDetailsTable . "(`business_Id`, `store_name`, `store_address1`, `store_address2`, `city`, `state_province`, `zip_code`, `country`, `phone_number`, `latitude`, `longitude`,`website_url`) VALUES ( " ;
	...
	public function createStoreDetails2 ($sqlInstance2, $storeDetails){
		$bInsert = -1;
		$bFound = false;
		//$this->sqlCreateStoreDetails = "INSERT INTO " . $database . "." . $this->storeDetailsTable . "(`business_Id`, `store_name`, `store_address1`, `store_address2`, `city`, `state_province`, `zip_code`, `country`, `phone_number`, `latitude`, `longitude`,`website_url`) VALUES ( " ;
             	$storeDetails->store_address1 = str_replace("'", "\\'", $storeDetails->store_address1);
             	$storeDetails->store_address1 = str_replace("&", "+", $storeDetails->store_address1);
             	$storeDetails->store_address2 = str_replace("'", "\\'", $storeDetails->store_address2);
             	$storeDetails->store_address2 = str_replace("&", "+", $storeDetails->store_address2);
             	$storeDetails->store_name     = str_replace("'", "\\'", $storeDetails->store_name);
             	$storeDetails->store_name     = str_replace("&", "+", $storeDetails->store_name);
             	$storeDetails->city           = str_replace("'", "\\'", $storeDetails->city);
             	$storeDetails->city           = str_replace("&", "+", $storeDetails->city);
        	$sqlString = $this->sqlCreateStoreDetails . "'" . $storeDetails->business_Id . "', '" . $storeDetails->store_name . "', '" . $storeDetails->store_address1 . "', '" . $storeDetails->store_address2 . "', '" . $storeDetails->city . "', '" . $storeDetails->city_Id . "', '" . $storeDetails->state_province . "', '" . $storeDetails->zip_code . "', '" . $storeDetails->country . "', '" . $storeDetails->phone_number . "', '" . $storeDetails->latitude . "', '" . $storeDetails->longitude . "', '" . $storeDetails->website_url . "', '" . $storeDetails->branch_Id . "');";
		$increment = 0;
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
	...
	public function createPkPassDetails2 ($sqlInstance2, $pkpassDetails){
		$bInsert = -1;
		$bFound = false;
        	$sqlString = $this->sqlCreatePkPassDetails . "'" . $pkpassDetails->graffiti_Id . "', '" . $pkpassDetails->city_Id . "', '" . $pkpassDetails->store_Id . "', '" . $pkpassDetails->org_name . "', '" . $pkpassDetails->org_desc . "', '" . $pkpassDetails->org_logo_text . "', '" . $pkpassDetails->coupon_type . "', '" . $pkpassDetails->background_color . "', '" . $pkpassDetails->foreground_color . "', '" . $pkpassDetails->primary_key . "', '" . $pkpassDetails->primary_label . "', '" . $pkpassDetails->primary_value . "', '" . $pkpassDetails->primary_text_alignment . "', '" .  $pkpassDetails->backfields_key . "', '" . $pkpassDetails->backfields_label . "', '" . $pkpassDetails->backfields_value . "', '" . $pkpassDetails->backfields_text_alignment . "', '" . $pkpassDetails->terms_value . "');";
		$increment = 0;
	...
	public function createCouponDetails ($couponDetails){
		$bInsert = -1;
		$bFound = false;
		//$this->sqlCreateCouponDetails = "INSERT INTO " . $database . "." . $this->couponDetailsTable . "(`graffiti_Id`, `city_Id`, `business_Id`, `coupon_url`, `swiggen_desc`, `expires`, `coupon_limit`, `latitude`, `longitude`, `nearest_store_Id`, `walking`) VALUES ( " ;
        	$sqlString = $this->sqlCreateCouponDetails . "'" . $couponDetails->graffiti_Id . "', '" . $couponDetails->city_Id . "', '" . $couponDetails->business_Id . "', '" . $couponDetails->business_name . "', '" . $couponDetails->coupon_url . "', '" . $couponDetails->swiggen_desc . "', '" . $couponDetails->expires . "', '" . $couponDetails->coupon_limit . "', '" . $couponDetails->latitude . "', '" . $couponDetails->longitude . "', '" . $couponDetails->nearest_store_Id . "', '" . $couponDetails->walking . "');";
		$increment = 0;
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
	...
	public function createCouponDetails2 ($sqlInstance2, $couponDetails, $version_copy){
		$bInsert = -1;
		$bFound = false;
		//$this->sqlCreateCouponDetails2 = "INSERT INTO " . $database . "." . $this->couponDetailsTable . "(`graffiti_Id`, `city_Id`, `business_Id`, `coupon_url`, `swiggen_desc`, `expires`, `coupon_limit`, `latitude`, `longitude`, `nearest_store_Id`, `walking`, `version_flag`) VALUES ( " ;
             	$couponDetails->swiggen_desc = str_replace("'", "\\'", $couponDetails->swiggen_desc);
             	$couponDetails->swiggen_desc = str_replace("&", "+"  , $couponDetails->swiggen_desc);
        	$sqlString = $this->sqlCreateCouponDetails2 . "'" . $couponDetails->graffiti_Id . "', '" . $couponDetails->city_Id . "', '" . $couponDetails->business_Id . "', '" . $couponDetails->business_name . "', '" . $couponDetails->coupon_url . "', '" . $couponDetails->swiggen_desc . "', '" . $couponDetails->expires . "', '" . $couponDetails->coupon_limit . "', '" . $couponDetails->latitude . "', '" . $couponDetails->longitude . "', '" . $couponDetails->nearest_store_Id . "', '" . $couponDetails->walking . "', '" . $version_copy . "');";
		$increment = 0;
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlCreateCouponDetails2 = [" . $sqlString . "]\n");
        	$res = mysqli_query($sqlInstance2->mysqli, $sqlString);
		if($res){
			$bInsert = mysqli_insert_id($this->mysqli);
			echo "Coupon Successfully Created [" . $couponDetails->graffiti_Id . "]\n";
...
$strSql = $this->sqlScriptAddCityId . "\"" . $city . "\", \"" . $state . "\", \"" . $country . "\", \"" . $country_abbr . "\", \"" . $first_latitude . "\", \"" . $first_longitude . "\");";
	...
	public function script_getCityId($city, $state, $country, $country_abbr, $first_latitude, $first_longitude){
        ...
        public function script_addCityId2($sqlInstance2, $city, $state, $country, $country_abbr, $first_latitude, $first_longitude, $geoNameId, $version){
                $bInsert = -1;
$strSql = $this->sqlScriptAddCityId2 . "\"" . $city . "\", \"" . $state . "\", \"" . $country . "\", \"" . $country_abbr . "\", \"" . $first_latitude . "\", \"" . $first_longitude . "\", \"" . $geoNameId . "\", \"" . $version . "\");";
                //if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
                        echo("strSql = [" . $strSql . "]\n");
                $res = mysqli_query($sqlInstance2->mysqli, $strSql);
                if($res){
        ...
        public function script_getCityId2($sqlInstance2, $city, $state, $country, $country_abbr, $first_latitude, $first_longitude, $geoNameId, $version){
                $bFoundId = false;
                //$strSql = $this->sqlGetCityId .  "CityName = \"" . $city . "\" AND CountryCode = \"" . $country_abbr . "\";"; 
                $strSql = $this->sqlGetCityId .  "CityName = \"" . $city . "\" AND " . " State = \"" . $state . "\" AND CountryCode = \"" . $country_abbr . "\";";
                //if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
                ...
                if($res){
                        ...
                        if($bFoundId == false)
                                $bFoundId = $this->script_addCityId2($sqlInstance2, $city, $state, $country, $country_abbr, $first_latitude, $first_longitude, $geoNameId, $version);
                }
                if($bFoundId == false || $bFoundId == -1)
                        echo("</cityId>");
                else
...
/* */
	...
	public function linkImage ($city_Id, $graffiti_Id, $user){
		...
		*/
        ...
        public function updateCouponDetails2($sqlInstance2, $couponDetails, $version){
		//$this->sqlCouponDetailsUpdateStr = "UPDATE " .  "`" . $database . "`" .  "." .  $this->couponDetailsTable . " SET 'version_flag' = "; //where author_alias  = 
		$strSql = $sqlInstance2->sqlCouponDetailsUpdateStr . " " . $version . " WHERE  graffiti_Id =" . $couponDetails->graffiti_Id . ";";
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("updateCouponDetails2:strSql = [" . $strSql . "]\n");
			$res = mysqli_query($sqlInstance2->mysqli, $strSql);
			if($res){
				echo("<updateCouponDetails>SUCCESS</updateCouponDetails>\n");
				$bInsert = mysqli_insert_id($sqlInstance2->mysqli);
				echo("<transaction_Id>" . $bInsert . "</transaction_Id>\n");
				return true;
			}
			else
				echo("<updateCouponDetails>FAIL</updateCouponDetails>\n");
		return false;
	}
...
//Replace the alternates to the regular fields and copy the existing fields to the alternate, as long as the Alternates are not null.
	...
	public function updateCityStateAlternatesOnly( $sqlInstance2, $city_Id, $CityNameAlternate, $StateAlternate, $GeoNameId){
		$mysqlUpdateCitiesStr = "UPDATE " .  "`" . $this->database . "`" .  "." .  $this->citiesTable . " SET ";
		if(is_null($CityNameAlternate) == FALSE)
			$mysqlUpdateCitiesStr = $mysqlUpdateCitiesStr . "`CityNameAlternate` = \"" . $CityNameAlternate . "\", "; 
		if(is_null($StateAlternate) == FALSE)
			$mysqlUpdateCitiesStr = $mysqlUpdateCitiesStr . "`StateAlternate` = \"" . $StateAlternate . "\", "; 
		$strSql2 = $mysqlUpdateCitiesStr .  " `GeoNameId` = \"" . $GeoNameId . "\" WHERE cityId = \"" . $city_Id . "\";"; 
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql2 = [" . $strSql2 . "]\n");
		$res = mysqli_query($sqlInstance2->mysqli, $strSql2);
		if($res){
			echo("<updateCityState>SUCCESS</updateCityState>\n");
			$bInsert = mysqli_insert_id($sqlInstance2->mysqli);
			echo("<transaction_Id>" . $bInsert . "</transaction_Id>\n");
	...
	public function isUserBanished8( $city_Id, $author_Id, $alias, $email){
		...
		if($res){
			if(mysqli_num_rows($res) > 0){
				...
				echo("<userBanished>FALSE</userBanished>\n");
				if(is_null($email) == TRUE || strlen($email) == 0)
					echo("<emailSame>TRUE</emailSame>\n");
				else if($this->isEmailTheSame($author_Id, $email) == FALSE)
					$this->updateEmailAddress($author_Id, $email);
				if($this->isAliasTheSame($author_Id, $alias) == FALSE)
...
//populates the Swiggen with the details below, joining both coupon_details with pkpass_details
	...
	public function getPkPassDetailsForSwiggen2( $sqlInstance2, $graffiti_Id, $city_Id, &$pkPassRowResult ){
		$strSql = "SELECT * FROM pkpass_details WHERE graffiti_Id =\"" . $graffiti_Id . "\" ;"; 
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");
		$res = mysqli_query($sqlInstance2->mysqli, $strSql);
		if($res){
			while($resultsArrayFetch = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				if(count($resultsArrayFetch) > 0) 
				{
					$pkPassRowResult->graffiti_Id 		= $resultsArrayFetch["graffiti_Id"];
					$pkPassRowResult->city_Id 		= $resultsArrayFetch["city_Id"];
					$pkPassRowResult->store_Id 		= $resultsArrayFetch["store_Id"];
					$pkPassRowResult->org_name 		= $resultsArrayFetch["org_name"];
					$pkPassRowResult->org_desc 		= $resultsArrayFetch["org_desc"];
					$pkPassRowResult->org_logo_text 	= $resultsArrayFetch["org_logo_text"];
					$pkPassRowResult->coupon_type 		= $resultsArrayFetch["coupon_type"];
					...
					$pkPassRowResult->backfields_text_alignment 		= $resultsArrayFetch["backfields_text_alignment"];
					$pkPassRowResult->terms_value 		= $resultsArrayFetch["terms_value"];
					//echo "read pkPassRowResult = [" . $pkPassRowResult . "]\n";
		    			mysqli_free_result($res);
					return true;
				}
			return true;
	...
	public function createStoresBackingCoupon2($sqlInstance2, $graffiti_Id, $city_Id, $store_Id, $business_Id, $nextSwiggenNumber){
		$bInsert = false;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("sqlDefineNextSwiggenNumber begin = " . $this->sqlDefineNextSwiggenNumber);
		$strSqlStep1 = sprintf("%d, %d, %d, %d, %d", $graffiti_Id, $city_Id, $store_Id, $business_Id, $nextSwiggenNumber);
	...
	public function script_getAllCities($sqlInstance2){
		$iCount = 0;
		$bFoundId = false;
		$strSql = "Select * from `cities` WHERE CityNameAlternate IS NULL AND GeoNameId IS NULL AND first_latitude IS NOT NULL AND first_longitude IS NOT NULL AND Version = \"1.8.01\" ORDER BY cityId ASC ; ";
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");
		$res = mysqli_query($this->mysqli, $strSql);
		if($res){
			while($newArray = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				$city_Id = $newArray['cityId'];
				$first_latitude = $newArray['first_latitude'];
				$first_longitude = $newArray['first_longitude'];
				$ExistingCityName = $newArray['CityName'];
				$ExistingState    = $newArray['State'];
				$CountryCode	  = $newArray['CountryCode'];
				echo("CityName=[" . $ExistingCityName . "], StateName = [" . $ExistingState . "]<cityId>" . $city_Id . ":[". $first_latitude . "],[". $first_longitude . "] </cityId>\n");
				//return $city_Id;
 				$geoCodeObj = new geoCode(NULL);
 				if($geoCodeObj->reverseGeoCode($first_latitude, $first_longitude)){
					if(strcmp($geoCodeObj->CountryCode, "US") != 0)//if it is not US then use the full name
						$geoCodeObj->StateProvinceShortName = $geoCodeObj->StateProvinceName;
					echo "geoCodeObject_CityAlternate = [" . $geoCodeObj->CityName . "], geoCodeObject_StateProvinceName = [". $geoCodeObj->StateProvinceShortName . "], GeoNameId = [". $geoCodeObj->GeoNameId . "]\n";
					$bSame = false;
					if(strcmp($ExistingCityName, $geoCodeObj->CityName)== 0 && 
				   	strcmp($ExistingState, $geoCodeObj->StateProvinceShortName)== 0 ){
						echo "they are the same, nulling out CityNameAlternate and StateAlternate, just putting in GeoNameId\n";
						$geoCodeObj->CityName 		     = NULL;
						$geoCodeObj->StateProvinceShortName  = NULL;
						$bSame = true;
					}
					else if(strcmp($ExistingCityName, $geoCodeObj->CityName)== 0){
						$geoCodeObj->CityName		     = NULL;
					}
					else if(strcmp($ExistingState, $geoCodeObj->StateProvinceShortName)== 0 ){
						$geoCodeObj->StateProvinceShortName  = NULL;
					}
					else 
						echo "they are not the same, will try to update next\n";
					if($iCount++ < 100 ){ //walking slowly through this
						$this->updateCityStateAlternatesOnly( $sqlInstance2, $city_Id, $geoCodeObj->CityName, $geoCodeObj->StateProvinceShortName, $geoCodeObj->GeoNameId);
						$sqlInstance2->reconnect();//reset this connection
					}
					else {
						echo "exiting out because of iCount >= 100\n";
						return;
					}
				}
				else
					echo "returned from reverseGeoCode with a false\n";
			}//end while
		}//end res
	}
// this routine takes each Starbucks and copies it to the appropriate new city name if it is no longer correct, any correct ones are skipped
	public function copyStarbucks($sqlInstance2){
		$iCount = 0;
		$bFoundId = false;
		$strSql = "Select * from `coupon_details` WHERE business_Id = 6 AND version_flag IS NULL ORDER BY city_Id, graffiti_Id ASC ; ";
		//$strSql = "Select * from `coupon_details` WHERE business_Id = 6 AND city_Id = 15 AND version_flag IS NULL ORDER BY city_Id, graffiti_Id ASC ; ";
		//$strSql = "Select * from `coupon_details` WHERE business_Id = 6 AND version_flag > 1.9 AND version_flag < 1.903 ORDER BY city_Id ASC ; ";
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");
		$res = mysqli_query($this->mysqli, $strSql);
		if($res){
 			$geoCodeObj = new geoCode(NULL);
			while($newArray = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				$couponDetails = new couponDetailsInstance(NULL);
				$couponDetails->graffiti_Id 	= $newArray['graffiti_Id'];
				$couponDetails->city_Id 	= $newArray['city_Id'];
				$couponDetails->business_name 	= $newArray['business_name'];
				$couponDetails->business_Id 	= $newArray['business_Id'];
				$couponDetails->coupon_url 	= $newArray['coupon_url'];
				$couponDetails->swiggen_desc 	= $newArray['swiggen_desc'];
             			$couponDetails->store_address2 = str_replace("'", "\\'", $couponDetails->swiggen_desc);
             			$couponDetails->store_address2 = str_replace("&", "+", $couponDetails->swiggen_desc);
				$couponDetails->currency_symbol	= $newArray['currency_symbol'];
				$couponDetails->expires		= $newArray['expires'];
				$couponDetails->coupon_limit	= $newArray['coupon_limit'];
				$couponDetails->downloaded_counter	= $newArray['downloaded_counter'];
				$couponDetails->latitude	= $newArray['latitude'];
				$couponDetails->longitude	= $newArray['longitude'];
				$couponDetails->nearest_store_Id= $newArray['nearest_store_Id'];
				$couponDetails->walking		= $newArray['walking'];
				echo("<graffiti_Id>".$couponDetails->graffiti_Id . "</graffiti_Id>\n");
				echo("<store_Id>".$couponDetails->nearest_store_Id . "</storeId>\n");
				echo("<CurrentCityId>" . $couponDetails->city_Id . ":[". $couponDetails->latitude . "],[". $couponDetails->longitude . "] </CurrentCityId>\n");
 				if($geoCodeObj->reverseGeoCode($couponDetails->latitude, $couponDetails->longitude)){
					$version = "1.801";
					$version_copy = "1.901";
					if($CountryCode != "US")
						$newCityId = $sqlInstance2->script_getCityId2($sqlInstance2, $geoCodeObj->CityName, $geoCodeObj->StateProvinceName, $geoCodeObj->CountryName, $geoCodeObj->CountryCode, $geoCodeObj->latitude, $geoCodeObj->longitude, $geoCodeObj->GeoNameId, $version);
					else
						$newCityId = $sqlInstance2->script_getCityId2($sqlInstance2, $geoCodeObj->CityName, $geoCodeObj->StateProvinceShortName, $geoCodeObj->CountryName, $geoCodeObj->CountryCode, $geoCodeObj->latitude, $geoCodeObj->longitude, $geoCodeObj->GeoNameId, $version);
					//is cityId different for this coupon
					echo "$newCityId is [" . $newCityId . "]\n";
					if($newCityId != $couponDetails->city_Id){
                				$graf  = array();
						if($this->get_my_message($sqlInstance2, $couponDetails->graffiti_Id, $graf) == false){
							echo "returning from get_my_message = [" . $graf . "]\n";
							echo "graffiti_transactions:  we didn't find graffiti_Id = [", $couponDetails->graffiti_Id . "]\n";
							exit;
						}
						//1. create a new graffiti_Id  
						$newGraffitiId = "-50";
						$newGraffitiId = $this->postGraffitiMessage3($sqlInstance2, $graf->graffiti_author_Id, $graf->icon_name, $graf->message, $newCityId, $geoCodeObj->CountryCode, $graf->envelope_Id, $graf->x_coordinate, $graf->y_coordinate, $graf->z_coordinate, $graf->message_visible_distance, $graf->created_datetime, $graf->expires_datetime);
						echo "new GraffitiId = [" . $newGraffitiId . "]\n";
						if($graf->graffiti_author_Id == ""){
							echo "exiting now because graf graffiti_author_Id is 0\n";
							var_dump($graf);
							exit;
						}
						//2. copy storeDetails with the newCityId
						$storeDetails = new storeDetailsInstance(NULL);
						$originalStoreId = $couponDetails->nearest_store_Id;
						$storeWhereClause = " WHERE store_Id = " . $couponDetails->nearest_store_Id . ";";
						if($this->getStoreDetails2($sqlInstance2, $storeDetails, $storeWhereClause) == false){
							echo "failed to locate store given storeDetails = [" . $storeDetails . "], and store_Id = ". $couponDetails->nearest_store_Id . "\n";
							exit;
						}
						$storeDetails->city_Id = $newCityId;
						$newStoreId = $this->createStoreDetails2 ($sqlInstance2, $storeDetails);
						if($newStoreId == -1){
							echo "failed to createStoreDetails2\n";
							exit;
						}
						$this->linkImage($newCityId, $newGraffitiId, $graf->graffiti_author_Id);

						//3. create a new couponDetails
						// first update the couponDetails version_flag to 8.1
						if($this->updateCouponDetails2($sqlInstance2, $couponDetails, $version) == false){
							echo "we failed to update CouponDetails\n";
							exit;
						}
						$originalCityId			= $couponDetails->city_Id;
						$originalGraffitiId		= $couponDetails->graffiti_Id;
						$couponDetails->city_Id          = $newCityId;
						$couponDetails->nearest_store_Id = $newStoreId;
						$couponDetails->graffiti_Id      = $newGraffitiId;
						//addCouponDetails
						//this is the one we will be using moving forward
						if($this->createCouponDetails2 ($sqlInstance2, $couponDetails, $version_copy) == false){
							echo "failed to createCouponDetails2 " . $couponDetails . "\n";
							exit;
						}

						//4. create a new pkpass_details
						$pkPassRowResult = new PkPassDetailsInstance(NULL);
						$alwaysWorksGraffitiId = 622;
						$alwaysWorksCityId = 112;
						if($this->getPkPassDetailsForSwiggen2($sqlInstance2,  $alwaysWorksGraffitiId, $alwaysWorksCityId, $pkPassRowResult ) == false){
							echo "couldn't find PkPass for graffiti_Id = " . $graffiti_Id . "\n";
							exit;
						}
						$pkPassRowResult->graffiti_Id = $newGraffitiId;
						$pkPassRowResult->store_Id    = $newStoreId;
						$pkPassRowResult->city_Id     = $newCityId;
						if($this->createPkPassDetails2 ($sqlInstance2, $pkPassRowResult) == false){
							echo "failed to createPkPassDetails2 \n";
							exit;
						}
						//getPkPass
						//5. create a new stores_backing_coupon
						$this->createStoresBackingCoupon2($sqlInstance2, $newGraffitiId, $newCityId, $newStoreId, $couponDetails->business_Id, $newCityId);
						echo "<STARBUCKS_CHANGED><oldGraffitiId>" . $originalGraffitiId . "</oldGraffitiId><newGraffitiId>" . $newGraffitiId . "</newGraffitiId>";
						echo "<oldCityId>". $originalCityId . "</oldCityId><newCityId>" . $newCityId . "</newCityId>";
						echo "<oldStoreId>" . $originalStoreId . "</oldStoreId><newStoreId>" . $newStoreId . "</newStoreId></STARBUCKS_CHANGED>\n\n";
//PETE
					}else {
						echo "newCityId =[" . $newCityId . "] == oldCityId[" . $couponDetails->city_Id . "] CityName = " . $geoCodeObj->CityName . "\n";
						//here to update CouponDetails2 because we already check that they are equal
						if($this->updateCouponDetails2($sqlInstance2, $couponDetails, $version) == false){
							echo "we failed to update CouponDetails\n";
							exit;
						}
					}
					if($iCount++ < 1000 ){ //walking slowly through this
						$sqlInstance2->reconnect();//reset this connection
					}
					else {
						echo "exiting out because of iCount >= 1000\n";
						return;
					}
				}//reverseGeoCode
				else
	...
	public function get_my_message($sqlInstance, $graffiti_Id, &$arrayResults){
        	$whereClause = "";
        	if($sqlInstance != NULL) {
		//$strSql = "UPDATE  `produdn8_KilroyWasHere`.`store_details`SET `city_Id`='" . $update_city_Id . "' WHERE  `store_details`.`store_Id` =" . $storeId . ";";
                        $selectStatement = sprintf("SELECT * from  `produdn8_KilroyWasHere`.`graffiti_transactions` WHERE `graffiti_Id` = '%d'", $graffiti_Id);
                	printf("select statement = %s\n", $selectStatement);
                	if($sqlInstance->getGraffitiMessages2($arrayResults, $selectStatement) == true){
				//echo "get_my_message: arrayResult = [" . $arrayResults . "]\n";
				return true;
			}
			else
				echo "get_my_message: failed\n";
        	} else
