...
class SqlUtils {
	...
	var $viewOrDismissTable 	= "`viewOrDismiss` ";
	var $optOutTable 		= "`optOut` ";
	var $sqlGetProducts;
	var $sqlGetMeetingId;
	...
	var $sqlGetGraffitiMessages_2;
	var $sqlGetGraffitiMessages_3;
	...
	var $sqlGetWhereAmI;
	var $sqlGetCityId;
	var $sqlGetCurrencyId;
	var $sqlAddCurrencyId;
	var $sqlGetCityIdDivision;
	var $sqlGetAllCities;
	var $sqlAddCityId;
	var $sqlAddCityId1;
	var $sqlAddCityId3;
	var $sqlAddCityIdDivision;
	...
	var $sqlUpdateCouponDetails;
	var $sqlCreateStoreDetails;
	var $sqlCreateStoreDetails1;
	var $sqlUpdateStoreDetails;
	var $sqlGetStoreId;
	var $sqlGetBrand;
	var $sqlCreateBrand;
	var $sqlCreateBrand1;
	...
	public function __construct($hostname, $user, $pass, $database){
		...
		$this->sqlCreateBrand1 = "INSERT INTO " . $database . "." . $this->BusinessTable . "(`brand_name`, `merchantUuid`, `merchantNameHyphenated`) VALUES ( ";
		$this->sqlUpdateBrand = "UPDATE " . $database . "." . $this->BusinessTable . " ";
		$this->sqlViewed = "INSERT INTO " . $database . "." . $this->viewOrDismissTable . "(`graffiti_Id`, `viewer_Id`, `viewed`) VALUES ( ";
		$this->sqlBuy = "INSERT INTO " . $database . "." . $this->viewOrDismissTable . "(`graffiti_Id`, `viewer_Id`, `bought`) VALUES ( ";
		$this->sqlIgnored = "INSERT INTO " . $database . "." . $this->viewOrDismissTable . "(`graffiti_Id`, `viewer_Id`, `ignored`) VALUES ( ";
		$this->sqlOptOut = "INSERT INTO " . $database . "." . $this->optOutTable . "(`graffiti_Id`, `viewer_Id`, `untilTime`) VALUES ( ";
		$this->sqlResetTrips = "UPDATE " . $database . "." . $this->optOutTable . " set `untilTime` = ";
		$this->sqlSelectOptOut = "SELECT * FROM " . $this->optOutTable ;
		$this->sqlGetProducts = "SELECT * FROM " . $this->productsTable ;
		$this->sqlGetGraffitiMessages_2 = "SELECT * FROM " . $database . "." . $this->graffitiMessageTable . " " ;
		$this->sqlGetGraffitiMessages_3 = "SELECT t2.business_name, t2.business_Id, t2.coupon_url, t2.coupon_url_orig, t2.swiggen_desc, t2.currency_symbol, t2.expires, t2.coupon_limit, t2.downloaded_counter, nearest_store_Id, t1.* FROM " . $database . "." . $this->graffitiMessageTable . " AS t1 INNER JOIN " . $database . "." . $this->couponDetailsTable . " AS t2 ON t1.graffiti_Id = t2.graffiti_Id " ;

		$this->sqlGetGraffitiMessages_6 = "SELECT t2.business_name, t2.business_Id, t2.coupon_url, t2.coupon_url_orig, t2.swiggen_desc, t2.expires, t2.coupon_limit, t2.downloaded_counter, nearest_store_Id, t1.graffiti_Id, t1.city_Id, t1.country_code, t1.dealId, t1.message, t1.icon_name, t1.x_coordinate, t1.y_coordinate, t1.message_visible_distance, t1.category   FROM ". $database . "." . $this->graffitiMessageTable . " AS t1 INNER JOIN " . $database . "." . $this->couponDetailsTable . " AS t2 ON t1.graffiti_Id = t2.graffiti_Id " ;
		...
		$this->sqlGetDealId = "SELECT graffiti_Id FROM " . $database . "." . $this->graffitiMessageTable ;
		...
		$this->sqlUpdateCouponExpires = "UPDATE " . $database . "." . $this->couponDetailsTable . " SET ";
		...
		$this->sqlCreateStoreDetails1 = "INSERT INTO " . $database . "." . $this->storeDetailsTable . "(`business_Id`, `store_name`, `store_address1`, `store_address2`, `city`, `city_Id`, `state_province`, `zip_code`, `country`, `phone_number`, `latitude`, `longitude`,`website_url`, `branch_Id`, `groupon_neighborhood`, `groupon_store_Id`, `uuid`) VALUES ( " ;
		$this->sqlUpdateStoreDetails = "UPDATE " . $database . "." . $this->storeDetailsTable . " SET ";
		//$this->sqlGetStoreDetails = "SELECT `store_Id` FROM " . $database . "." . $this->storeDetailsTable . " WHERE ";
		$this->sqlCreateCouponDetails = "INSERT INTO " . $database . "." . $this->couponDetailsTable . "(`graffiti_Id`, `city_Id`, `business_Id`, `business_name`, `coupon_url`, `coupon_url_orig`, `swiggen_desc`, `expires`, `coupon_limit`, `latitude`, `longitude`, `nearest_store_Id`, `walking`, `downloaded_counter` ) VALUES ( " ;
		$this->sqlUpdateCouponDetails = "UPDATE " . $database . "." . $this->couponDetailsTable . " SET ";
		...
		$this->sqlPostGraffitiMessage3 = "INSERT INTO " . $database . "." . $this->graffitiMessageTable . " (`graffiti_author_Id`, `icon_name`, `message`, `city_Id`, `country_code`, `envelope_Id`, `x_coordinate`, `y_coordinate`, `z_coordinate`, `message_visible_distance`, `created_datetime`, `expires_datetime`, `dealId`, `category` ) VALUES ( " ;
		$this->sqlUpdateGraffitiMessage3 = "UPDATE " . $database . "." . $this->graffitiMessageTable . " SET"; 
		...
		$this->sqlGetCurrencyId = "SELECT currency_Id FROM " . $database . "." . $this->currencyTable . " WHERE ";
		$this->sqlAddCurrencyId = "INSERT INTO " . $database . "." . $this->currencyTable . " ( `currency_code` , `countryCode`) VALUES ( ";
		$this->sqlGetCityIdDivision = "SELECT cityId, first_latitude, first_longitude FROM " . $database . "." . $this->citiesTable . " WHERE ";
		...
		$this->sqlAddCityIdDivision= "INSERT INTO " . $database . "." . $this->citiesTable . "( `CityName` , `State` , `StateAlternate`, `Country`, `CountryCode`, `first_latitude`, `first_longitude`, `GeoNameId`, `County`) VALUES (";
		...
		$this->sqlUpdateCitiesDivisionStr = "UPDATE " .  "`" . $database . "`" .  "." .  $this->citiesTable . " "; 
...
//SELECT * FROM pkpass_details as t1 inner join coupon_details as t2 on t1.graffiti_Id = t2.graffiti_Id WHERE t1.graffiti_Id = 382;
		...
		$this->sqlLargestCities = "SELECT * from " . $database . "." . $this->citiesTable . " WHERE ";
	}
...
//UPDATE `produdn8_Graffiti`.`whereAmI` SET `x_location` = '20', `y_location` = '40', `last_mobile_request_time` = '', `last_mse_refresh_time` = NOW() WHERE `whereAmI`.`macAddress` = '00:00:00:00:00:01';
	...
	public function postGraffitiMessage3($username, $icon_name, $message, $city_Id, $country_code, $envelope_Id, $x, $y, $z, $away, $date, $until, $dealId, $categoryIndex){
		$bInsert = false;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("sqlPostGraffitiMessage begin = " . $this->sqlPostGraffitiMessage3);

		$strSqlStep1 = sprintf("'%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s', '%s', '%d', '%s', '%s' , '%s', '%d'", $username, $icon_name, $message, $city_Id, $country_code, $envelope_Id, $x, $y, $z, $away, $date, $until, $dealId, $categoryIndex);
		//$strSql = $this->sqlPostGraffitiMessage2 . $strSqlStep1 . $strSqlStep2 . $strSqlStep3 ");";
	...
	public function updateGraffitiMessage3($graffiti_Id, $username, $icon_name, $message, $city_Id, $country_code, $envelope_Id, $x, $y, $z, $away, $date, $until, $dealId, $categoryIndex){
		$bUpdate = false;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("sqlUpdateGraffitiMessage begin = " . $this->sqlUpdateGraffitiMessage3);
//(`graffiti_author_Id`, `icon_name`, `message`, `city_Id`, `country_code`, `envelope_Id`, `x_coordinate`, `y_coordinate`, `z_coordinate`, `message_visible_distance`, `created_datetime`, `expires_datetime`, `dealId`, `category` ) VALUES ( " ;

		$strSqlStep1 = sprintf(" `graffiti_author_id` = '%s', `icon_name` = '%s', `message` = '%s', `city_Id` = '%s', `country_code` = '%s', `envelope_Id` = '%d', `x_coordinate` = '%s', `y_coordinate` = '%s', `z_coordinate` = '%s', `message_visible_distance` = '%d', `created_datetime` = '%s', `expires_datetime` = '%s' , `dealId` = '%s', `category` = '%d' WHERE `graffiti_Id` = '%s' ;", $username, $icon_name, $message, $city_Id, $country_code, $envelope_Id, $x, $y, $z, $away, $date, $until, $dealId, $categoryIndex, $graffiti_Id);
		$strSql = $this->sqlUpdateGraffitiMessage3 . $strSqlStep1;
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("sqlUpdateGraffitiMessage3.strSql = [" . $strSql . "]\n");
		$res = mysqli_query($this->mysqli, $strSql);
		if($res){
			$bUpdate = TRUE;
		} else {
			error_log("strSql = [" . $strSql . "]\n" , 3, "/home1/produdn8/etc/groupon/UpdateGraffiti.log");
			echo("failed to update message: " . mysqli_error($this->mysqli) . "]\n" );
		}
		return $bUpdate;
	}
...
//uses the FilterArray to remove elements that that user doesn't want to be notified about
	public function getGraffitiMessages_2 (&$arrayResults, $whereClause, $optOutFilterArray){
		$bFound = false;
        $sqlString = $this->sqlGetGraffitiMessages_2 . $whereClause;
		$increment = 0;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlGetGraffitiMessages = [" . $sqlString . "]\n");
        $res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			while($resultsArrayFetch = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				if(count($resultsArrayFetch) > 0) 
				{
					$testGraffitiId = $resultsArrayFetch['graffiti_Id'];
					//echo("checking testGraffitiId = " . $testGraffitiId . "\n");
					//var_dump($optOutFilterArray);
					if(in_array($testGraffitiId, $optOutFilterArray) == FALSE)
						//echo "skipping testGraffitiId = [" . $testGraffitiId . "]\n";
						//else 
					{
						$productRowResult = new graffitiMessageInstance(NULL);
						$productRowResult->city_Id    		= $resultsArrayFetch['city_Id'];
						$productRowResult->graffiti_Id    	= $testGraffitiId;
						$productRowResult->envelope_Id    	= $resultsArrayFetch['envelope_Id'];
						$productRowResult->graffiti_author_Id   = $resultsArrayFetch['graffiti_author_Id'];
						$productRowResult->country_code   	= $resultsArrayFetch['country_code'];
						$productRowResult->created_datetime 	= $resultsArrayFetch['created_datetime'];
						...
						$productRowResult->z_coordinate 	= $resultsArrayFetch['z_coordinate'];
						$productRowResult->message_visible_distance = $resultsArrayFetch['message_visible_distance'];
						$productRowResult->category 		= $resultsArrayFetch['category'];
						$bFound = true;
						array_push($arrayResults,$productRowResult);
					}
				} else
...
//uses the FilterArray to remove elements that that user doesn't want to be notified about
	public function getGraffitiMessages_3 (&$arrayResults, $whereClause, $optOutFilterArray){
		$bFound = false;
        $sqlString = $this->sqlGetGraffitiMessages_3 . $whereClause;
		$increment = 0;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlGetGraffitiMessages = [" . $sqlString . "]\n");
        $res = mysqli_query($this->mysqli, $sqlString);
				...
				if(count($resultsArrayFetch) > 0) 
				{
						...
						//else 
					{
						...
						$productRowResult->category 		= $resultsArrayFetch['category'];
        					...
        					$productRowResult->coupon_url_orig      = $resultsArrayFetch['coupon_url_orig'];
        					...
        					$productRowResult->downloaed_counter 	= $resultsArrayFetch['downloaded_counter'];
	...
	public function getGraffitiMessages_4 (&$arrayResults, $whereClause, $optOutFilterArray){
		$bFound = false;
        $sqlString = $this->sqlGetGraffitiMessages_3 . $whereClause;
		$increment = 0;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
	...
	public function getGraffitiMessages_6 (&$arrayResults, $whereClause){
		$bFound = false;
        	$sqlString = $this->sqlGetGraffitiMessages_6 . $whereClause;
		$increment = 0;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlGetGraffitiMessages_6 = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			while($resultsArrayFetch = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				if(count($resultsArrayFetch) > 0) 
				{
					...
					$myProductRowResult = new graffitiMessageInstance6(NULL);
					$testGraffitiId 			= $resultsArrayFetch['graffiti_Id'];
					$myProductRowResult->city_Id    	= $resultsArrayFetch['city_Id'];
					$myProductRowResult->graffiti_Id    	= $testGraffitiId;
					$myProductRowResult->dealId 		= $resultsArrayFetch['dealId'];
					$myProductRowResult->message 		= $resultsArrayFetch['message'];
					$myProductRowResult->icon_name 		= $resultsArrayFetch['icon_name'];
					$myProductRowResult->x_coordinate 	= $resultsArrayFetch['x_coordinate'];
					$myProductRowResult->y_coordinate 	= $resultsArrayFetch['y_coordinate'];
					$myProductRowResult->message_visible_distance = $resultsArrayFetch['message_visible_distance'];
					$myProductRowResult->category 		= $resultsArrayFetch['category'];
        				$myProductRowResult->business_name 	= $resultsArrayFetch['business_name'];
        				$myProductRowResult->business_Id 	= $resultsArrayFetch['business_Id'];
        				$myProductRowResult->coupon_url 	= $resultsArrayFetch['coupon_url'];
        				$myProductRowResult->coupon_url_orig    = $resultsArrayFetch['coupon_url_orig'];
        				$myProductRowResult->swiggen_desc 	= $resultsArrayFetch['swiggen_desc'];
        				$myProductRowResult->expires 		= $resultsArrayFetch['expires'];
        				$myProductRowResult->coupon_limit 	= $resultsArrayFetch['coupon_limit'];
        				$myProductRowResult->downloaded_counter = $resultsArrayFetch['downloaded_counter'];
        				$myProductRowResult->nearest_store_Id 	= $resultsArrayFetch['nearest_store_Id'];
					$bFound = true;
					array_push($arrayResults,$myProductRowResult);
					//echo("\n[productRowResult]:" . $productRowResult . "\n");
				} else
					echo("failed to locate graffitiMessages6\n");
		    }//while
		    mysqli_free_result($res);
		    return true;
		} else {
	...
	public function getGraffitiMessages_3_OriginalCoupon (&$arrayResults, $whereClause){
		$bFound = false;
        $sqlString = $this->sqlGetGraffitiMessages_3 . $whereClause;
		$increment = 0;
        $res = mysqli_query($this->mysqli, $sqlString);
	...
	public function getGraffitiDealIdAndGraffitiId ($dealId, &$graffiti_Id, $storeLatitude, $storeLongitude){
		$bFound = false;
        $sqlString = $this->sqlGetDealId . " WHERE `dealId` = \"" . $dealId . "\" AND `x_coordinate` = '" . $storeLatitude . "' AND `y_coordinate` = '" . $storeLongitude . "';";
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		echo("sqlGetGraffitiDealId = [" . $sqlString . "]\n");
...
*/
	...
	public function getOptOutDetails (&$arrayResults, $whereClause){
		$bFound = false;
		//printf("sqlGetStoreDetails = [%s]\n", $this->sqlGetStoreDetails);
        	$sqlString = $this->sqlSelectOptOut . $whereClause;
		$increment = 0;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlGetOptOutDetails = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			while($resultsArrayFetch = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				if(count($resultsArrayFetch) > 0) 
				{
					//only populate the graffiti_Id you need to filter out for later
					$arrayResults[] = $resultsArrayFetch['graffiti_Id'];
					if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
						echo "graffiti_Id matched = [" . $resultsArrayFetch['graffiti_Id'] . "]\n";
/*
					$productRowResult = new optOutInstance(NULL);
					$productRowResult->graffiti_Id    	= $resultsArrayFetch['graffiti_Id'];
*/
					$bFound = true;
...
*/
	...
	public function i_OptOut ($graffiti_Id, $viewer_Id, $days){
		$bInsert = -1;
		$nowTime = date(); 
		$futureTime = date();
		$this->getNowPlusDays($days, $nowTime, $expiresTime);
		//$this->sqlOptOut = "INSERT INTO " . $database . "." . $this->optOutTable . "(`graffiti_Id`, `viewer_Id`, `untilTime`) VALUES ( ";
        	$sqlString = $this->sqlOptOut . "'" . $graffiti_Id . "', '" . $viewer_Id . "', '" . $expiresTime . "');";
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sql OptOut = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			$transaction_Id = mysqli_insert_id($this->mysqli);
			if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
				echo "Transaction_Id now = [" . $transaction_Id . "]\n";
                        echo("<iOptOut>SUCCESS</iOptOut>\n");
			return true;
		}
		else {
			$errorMsg =  "<iOptOut>failed to insert using string [" . $sqlString . "]</iOptOut>\n";
			error_log($errorMsg . mysqli_error($this->mysqli) . "]\n" , 3, "/home1/produdn8/etc/groupon/errorMsg.log");
			echo $errorMsg;
		}
		return false;
	}

	//this clears everything from OptOut table and the ignore
	public function i_ResetTrips ($viewer_Id){
		$bInsert = -1;
		$nowTime = date(); 
		$futureTime = date();
		$this->getNow($nowTime);
		//$this->sqlRestTrips = "UPDATE " . $database . "." . $this->optOutTable . " set `untilTime` = ";
        	$sqlString = $this->sqlResetTrips . "'" . $nowTime . "' WHERE `viewer_Id` = " . $viewer_Id . " ;";
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sql ResetTrips = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			$row_cnt = mysqli_num_rows($res);
			if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
				echo "num rows affected now = [" . $row_cnt . "]\n";
                        echo("<iResetTrips>SUCCESS</iResetTrips>\n");
			return true;
		}
		else {
			$errorMsg =  "<iResetTrips>failed to update using string [" . $sqlString . "]</iResetTrips>\n";
			error_log("<iResetTrips> failed to run the command</iResetTrips> [" . $errorMsg . mysqli_error($this->mysqli) . "]\n" , 3, "/home1/produdn8/etc/groupon/errorMsg.log");
			echo $errorMsg;
		}
		return false;
	}
	public function i_Gnored ($graffiti_Id, $viewer_Id){
		$bInsert = -1;
        	$sqlString = $this->sqlIgnored . "'" . $graffiti_Id . "', '" . $viewer_Id . "', 'true');";
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sql iGnored = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			$transaction_Id = mysqli_insert_id($this->mysqli);
			if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
				echo "Transaction_Id now = [" . $transaction_Id . "]\n";
			echo("<iGnored>SUCCESS</iGnored>\n");
			return true;
		}
		else {
			$errorMsg =  "<iGnored>failed to insert using string [" . $sqlString . "]</iGnored>\n";
			error_log($errorMsg . mysqli_error($this->mysqli) . "]\n" , 3, "/home1/produdn8/etc/groupon/errorMsg.log");
			echo $errorMsg;
		}
		return false;
	...
	public function i_Viewed ($graffiti_Id, $viewer_Id){
		$bInsert = -1;
        	$sqlString = $this->sqlViewed . "'" . $graffiti_Id . "', '" . $viewer_Id . "', 'true');";
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sql iViewed = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			$transaction_Id = mysqli_insert_id($this->mysqli);
			if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
				echo "Transaction_Id now = [" . $transaction_Id . "]\n";
			echo("<iViewed>SUCCESS</iViewed>\n");
			return true;
		}
		else {
			$errorMsg =  "<iViewed>failed to insert using string [" . $sqlString . "]</iViewed>\n";
			error_log($errorMsg . mysqli_error($this->mysqli) . "]\n" , 3, "/home1/produdn8/etc/groupon/errorMsg.log");
			echo $errorMsg;
		}
		return false;
	}
       public function i_Buy ($graffiti_Id, $viewer_Id){
                $bInsert = -1;
                $sqlString = $this->sqlBuy . "'" . $graffiti_Id . "', '" . $viewer_Id . "', 'true');";
                if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
                   echo("sql iBuy = [" . $sqlString . "]\n");
                $res = mysqli_query($this->mysqli, $sqlString);
                if($res){
                        $transaction_Id = mysqli_insert_id($this->mysqli);
                        if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
                                echo "Transaction_Id now = [" . $transaction_Id . "]\n";
                        echo("<iBuy>SUCCESS</iBuy>\n");
                        return true;
                }
                else {
                        $errorMsg =  "<iBuy>failed to insert using string [" . $sqlString . "]</iBuy>\n";
                        error_log($errorMsg . mysqli_error($this->mysqli) . "]\n" , 3, "/home1/produdn8/etc/groupon/errorMsg.log");
                        echo $errorMsg;
                }
                return false;
...
//PETEY HERE
	public function createBusinessIdGivenUuid (&$business_Id, $brand_name, $merchantUuid, $merchantNameHyphenated){
		$bInsert = -1;
		//$this->sqlCreateBrand = "INSERT INTO " . $database . "." . $this->BusinessTable . "(`brand_name`) VALUES ( ";
        	$sqlString = $this->sqlCreateBrand1 . "'" . $brand_name . "', '" . $merchantUuid . "', '" . $merchantNameHyphenated . "');";
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlCreateBrand1 = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			$business_Id = mysqli_insert_id($this->mysqli);
			if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
	...
	public function updateBusinessId($business_Id, $brand_name, $merchantUuid, $merchantNameHyphenated){
        	$sqlString = $this->sqlUpdateBrand . " SET `brand_name` = '" . $brand_name . "', `merchantUuid` = '" . $merchantUuid . "', `merchantNameHyphenated` = '" . $merchantNameHyphenated . "' WHERE business_Id = '" . $business_Id . "' ;";
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlCreateBrand1 = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			echo "BusinessId Successfully Updated [" . $business_Id . "]\n";
			return true;
		}
		else
			echo "failed to update businessId using string [" . $sqlString . "]\n";
		return false;
	}
	public function getBusinessIdFromBrandNameAndUuid (&$business_Id, $brand_name, $merchantUuid, $merchantHyphenatedName, &$exitingUuid){
		$bFound = false;
                $sqlString = $this->sqlGetBrand . " `brand_name` = '" .  $brand_name . "' OR `merchantUuid` = '" . $merchantUuid . "';";
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlGetBusinessIdFromBrandNameAndUuid = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			while($resultsArrayFetch = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				if(count($resultsArrayFetch) > 0) 
				{
					$business_Id    	= $resultsArrayFetch['business_Id'];
					$existingUuid    	= $resultsArrayFetch['merchantUuid'];
					if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
						echo "business_Id now = [" . $business_Id . "]\n";
					return true;
				}
	...
	public function createStoreDetails1 ($storeDetails){
		$bInsert = -1;
		$bFound = false;
		//$this->sqlCreateStoreDetails1 = "INSERT INTO " . $database . "." . $this->storeDetailsTable . "(`business_Id`, `store_name`, `store_address1`, `store_address2`, `city`, `city_Id`, `state_province`, `zip_code`, `country`, `phone_number`, `latitude`, `longitude`,`website_url`, `branch_Id`, `groupon_neighborhood`, `groupon_store_Id`, `uuid`) VALUES ( " ;
        	$sqlString = $this->sqlCreateStoreDetails1 . "'" . $storeDetails->business_Id . "', '" . $storeDetails->store_name . "', '" . $storeDetails->store_address1 . "', '" . $storeDetails->store_address2 . "', '" . $storeDetails->city . "', '" . $storeDetails->city_Id . "', '" . $storeDetails->state_province . "', '" . $storeDetails->zip_code . "', '" . $storeDetails->country . "', '" . $storeDetails->phone_number . "', '" . $storeDetails->latitude . "', '" . $storeDetails->longitude . "', '" . $storeDetails->website_url . "', '" . $storeDetails->branch_Id . "', '" . $storeDetails->groupon_neighborhood . "', '" . $storeDetails->groupon_store_Id . "', '" . $storeDetails->uuid . "' );";
		$increment = 0;
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlCreateStoreDetails1 = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			$bInsert = mysqli_insert_id($this->mysqli);
			echo "StoreId now = [" . $bInsert . "]\n";
	...
	public function updateStoreDetails ($storeDetails){
		$bUpdate = -1;
		$sqlString = sprintf("%s `business_Id` = '%d', `store_name` = '%s', `store_address1` = '%s', `store_address2` = '%s', `city` = '%s', `city_Id` = '%s', `state_province` = '%s', `zip_code` = '%s', `country` = '%s', `phone_number` = '%s', `latitude` = '%s', `longitude` = '%s', `website_url` = '%s' , `branch_Id` = '%s' WHERE `store_Id` = '%s' ;", 
		$this->sqlUpdateStoreDetails, 
        	$storeDetails->business_Id, 
		$storeDetails->store_name, 
		$storeDetails->store_address1, 
		$storeDetails->store_address2,
		$storeDetails->city,
		$storeDetails->city_Id,
		$storeDetails->state_province,
		$storeDetails->zip_code,
		$storeDetails->country,
		$storeDetails->phone_number,
		$storeDetails->latitude,
		$storeDetails->longitude,
		$storeDetails->website_url,
		$storeDetails->branch_Id,
		$storeDetails->store_Id);
		$increment = 0;

		//$sqlString = $sqlUpdateStoreDetails . $strSqlStep1;
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlUpdateStoreDetails = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			echo "Store Successfully Updated [" . $storeDetails->store_Id . "]\n";
			$bUpdate = TRUE;
		}
		else
			echo "failed to update Store using sql [" . $sqlString . "]\n";
		return $bUpdate;
	}
	public function updateStoreDetails1 ($storeDetails){
		$bUpdate = -1;
		$sqlString = sprintf("%s `business_Id` = '%d', `store_name` = '%s', `store_address1` = '%s', `store_address2` = '%s', `city` = '%s', `city_Id` = '%s', `state_province` = '%s', `zip_code` = '%s', `country` = '%s', `phone_number` = '%s', `latitude` = '%s', `longitude` = '%s', `website_url` = '%s' , `branch_Id` = '%s' , `groupon_store_Id` = '%s', `groupon_neighborhood` = '%s', `uuid` = '%s' WHERE `store_Id` = '%s' ;", 
		$this->sqlUpdateStoreDetails, 
        	$storeDetails->business_Id, 
		$storeDetails->store_name, 
		$storeDetails->store_address1, 
		...
		$storeDetails->website_url,
		$storeDetails->branch_Id,
		$storeDetails->groupon_store_Id, 
		$storeDetails->groupon_neighborhood,
		$storeDetails->uuid,
		$storeDetails->store_Id);
		$increment = 0;
		...
		//$sqlString = $sqlUpdateStoreDetails . $strSqlStep1;
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlUpdateStoreDetails1 = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			echo "Store Successfully Updated [" . $storeDetails->store_Id . "]\n";
			$bUpdate = TRUE;
	...
	public function getStoreIdAndName (&$store_name, &$store_Id, $latitude, $longitude){
		$bInsert = -1;
		$bFound = false;
        	$sqlString = $this->sqlGetStoreDetails . " WHERE `latitude` = " . $latitude . " AND `longitude` = " . $longitude .  ";";
		$increment = 0;
		...
		if($res){
			while($resultsArrayFetch = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				...
				{
					$store_Id    	= $resultsArrayFetch['store_Id'];
					$store_name	= $resultsArrayFetch['store_name'];
					return TRUE;
				}
			}
		}
	...
	public function createCouponDetails ($couponDetails){
		$bInsert = -1;
		$bFound = false;
        	$sqlString = $this->sqlCreateCouponDetails . "'" . $couponDetails->graffiti_Id . "', '" . $couponDetails->city_Id . "', '" . $couponDetails->business_Id . "', '" . $couponDetails->business_name . "', '" . $couponDetails->coupon_url . "', '" . $couponDetails->coupon_url_orig . "', '" . $couponDetails->swiggen_desc . "', '" . $couponDetails->expires . "', '" . $couponDetails->coupon_limit . "', '" . $couponDetails->latitude . "', '" . $couponDetails->longitude . "', '" . $couponDetails->nearest_store_Id . "', '" . $couponDetails->walking . "', '" . $couponDetails->downloaded_counter . "');";
		$increment = 0;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
	...
	public function updateCouponDetails ($couponDetails){
		$bUpdate = FALSE;
//(`graffiti_Id`, `city_Id`, `business_Id`, `business_name`, `coupon_url`, `coupon_url_orig`, `swiggen_desc`, `expires`, `coupon_limit`, `latitude`, `longitude`, `nearest_store_Id`, `walking`, `downloaded_counter` ) VALUES ( " ;
		$strSqlStep1 = sprintf(" `city_Id` = '%s', `business_Id` = '%d', `business_name` = '%s', `coupon_url` = '%s', `coupon_url_orig` = '%s', `swiggen_desc` = '%s', `expires` = '%s', `coupon_limit` = '%s', `latitude` = '%s', `longitude` = '%s', `nearest_store_Id` = '%s' , `walking` = '%s', `downloaded_counter` = '%d' WHERE `graffiti_Id` = '%s' ;", 
			$couponDetails->city_Id, 
			$couponDetails->business_Id, 
			$couponDetails->business_name, 
			$couponDetails->coupon_url, 
			$couponDetails->coupon_url_orig, 
 			$couponDetails->swiggen_desc,
			$couponDetails->expires,
			$couponDetails->coupon_limit,
			$couponDetails->latitude,
			$couponDetails->longitude,
			$couponDetails->nearest_store_Id,
			$couponDetails->walking,
			$couponDetails->downloaded_counter,
			$couponDetails->graffiti_Id);
        	$sqlString = $this->sqlUpdateCouponDetails . $strSqlStep1;
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
		   echo("sqlUpdateCouponDetails = [" . $sqlString . "]\n");
        	$res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			echo "Coupon Successfully Updated [" . $couponDetails->graffiti_Id . "]\n";
			$bUpdate = TRUE;
		}
		else {
			echo "failed to update coupon_details table using string [" . $sqlString . "]\n";
			$bUpdate = FALSE;
		}
		return $bUpdate;
	}
	...
	public function getNowPlusDays ($days, &$timeStamp, &$timeStampPlus){
		$bFound = false;
                //$sqlString = "SELECT NOW() from graffiti_transactions limit 1;";
		$dayIncrement = "DATE_ADD(NOW(), INTERVAL " . $days . " DAY)";
                $sqlString = "SELECT NOW(), " . $dayIncrement . ";";
		$increment = 0;
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			printf("getNowPlusDays = [%s]\n", $sqlString);
                $res = mysqli_query($this->mysqli, $sqlString);
		if($res){
			while($resultsArrayFetch = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				$bFound = true;
				$timeStamp = $resultsArrayFetch['NOW()'];
				$timeStampPlus = $resultsArrayFetch[$dayIncrement];
				if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
					echo "timeStamp now = [" . $timeStamp . "], timeStampPlus = [" . $timeStampPlus . "]\n";
			}
		    	mysqli_free_result($res);
...
$strSql = $this->sqlScriptAddCityId . "\"" . $city . "\", \"" . $state . "\", \"" . $country . "\", \"" . $country_abbr . "\", \"" . $first_latitude . "\", \"" . $first_longitude . "\");";
	...
	public function largestPopulationCitiesPerState(&$citiesArray, $state, $country_abbr, $pop){
		$bFoundId = false;
		$strSql = $this->sqlLargestCities .  " excludeBigCity is NULL AND population > '" . $pop . "' AND " . " State = '" . $state . "' AND CountryCode = '" . $country_abbr . "';"; 
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");
		$res = mysqli_query($this->mysqli, $strSql);
		if($res){
			while($theArray = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				$cityInstance = new citiesInstance(NULL);
				$cityInstance->cityId = $theArray['cityId'];
				echo("<cityId>" . $cityInstance->cityId . "</cityId>");

				$cityNameStep1 = $theArray['CityName'];
				echo("cityName was [" . $cityNameStep1 . "]:");
				$cityNameStep2 = str_replace(". ", '', $cityNameStep1);
				$cityNameStep3 = str_replace(" ", '-', $cityNameStep2);
				$cityInstance->CityName = strtolower($cityNameStep3);
				echo("now [" . $cityInstance->CityName . "]\n");

				$cityInstance->State = $theArray['State'];
				$cityInstance->population = $theArray['population'];
				$cityInstance->first_latitude = $theArray['first_latitude'];
				$cityInstance->first_longitude = $theArray['first_longitude'];
                                array_push($citiesArray,$cityInstance);
			}
			return true;
		}
		return $false;
	}
	public function largestPopulationCities(&$citiesArray, $country_abbr, $pop){
		$bFoundId = false;
		$strSql = $this->sqlLargestCities .  " excludeBigCity is NULL AND population > '" . $pop . "' AND CountryCode = '" . $country_abbr . "';"; 
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");
		$res = mysqli_query($this->mysqli, $strSql);
		if($res){
			while($theArray = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				...
				//$cityInstance->CityName = $theArray['CityName'];
				$cityInstance->State = $theArray['State'];
				$cityInstance->population = $theArray['population'];
				$cityInstance->first_latitude = $theArray['first_latitude'];
				$cityInstance->first_longitude = $theArray['first_longitude'];
...
//PETER CURRENCY
	public function addCurrencyId(&$newCurrency_Id, $currency_code, $countryCode){
		$strSql = $this->sqlAddCurrencyId . "\"" . $currency_code . "\", \"" . $countryCode . "\");";
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");

		error_log("strSql = [" . $strSql . "]\n" , 3, "/home1/produdn8/etc/groupon/NewCurrency.1.log");
		$res = mysqli_query($this->mysqli, $strSql);
		if($res){
			$bInsert = mysqli_insert_id($this->mysqli);
			$newCurrency_Id = $bInsert;
			echo("<currency_Id>" . $bInsert . "</currency_Id>");
			error_log("<currency_Id>" . $bInsert . "</currency_Id>" , 3, "/home1/produdn8/etc/groupon/NewCurrency.1.log");
			return true;	
		}
		return false;
	}
	public function getCurrencyId(&$currency_Id, $currency_code, $countryCode){
		$strSql = $this->sqlGetCurrencyId .  "`currency_code` = \"" . $currency_code . "\" AND `countryCode` = \"" . $countryCode . "\";"; 
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");
		$res = mysqli_query($this->mysqli, $strSql);
		if($res){
			while($newArray = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				$currency_Id 	= $newArray['currency_Id'];
				return true;
			}
		}
		echo "failed to locate currency_code = " . $currency_code . " and countryCode = " . $countryCode . "\n";
		echo "adding it now\n";
		$newCurrency_Id = -1;
		$this->addCurrencyId($newCurrency_Id, $currency_code, $countryCode);
		$currency_Id = $newCurrency_Id;
		return true;
	}
...
*/
	...
	public function addCityIdUsingDivision(&$city_Id, $city, $state, $shortState, $country, $country_abbr, $latitude, $longitude, $geoNameId, $county, $grouponSearchTerm){
		$strSql = $this->sqlAddCityIdDivision . "\"" . $city . "\", \"" . $state . "\", \"" . $shortState . "\", \"" . $country . "\", \"" . $country_abbr . "\", \"" . $latitude . "\", \"" . $longitude . "\", \"" . $geoNameId . "\, \"" . $county . "\, \"" . $grouponSearchTerm . "\" );";
	//	if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");

		error_log("strSql = [" . $strSql . "]\n" , 3, "/home1/produdn8/etc/groupon/NewCityLookup.usingDivision.log");
		...
		if($res){
			...
			$city_Id = $bInsert;
			echo("<cityId>" . $bInsert . "</cityId>");
			error_log("<cityId>" . $bInsert . "</cityId>" , 3, "/home1/produdn8/etc/groupon/NewCityLookup.usingDivision.log");
			return true;	
		}
		return false;
	}
	...
	public function getAllCitiesByStateAndCountryCode(&$arrayResults, $stateAbbr, $countryCode){
		$bFound = false;
		$strSql = $this->sqlGetAllCities .  " `State` ='" . $stateAbbr . "' AND `CountryCode` = '" . $countryCode . "' AND `IGNORE` IS NULL ORDER BY `CityName`;"; 
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");
		$res = mysqli_query($this->mysqli, $strSql);
		if($res){
			...
			while($resultsArrayFetch = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				if(count($resultsArrayFetch) > 0) 
				{
					$productRowResult = new geoCodeInstance(NULL);
					$productRowResult->GeoNameId    = $resultsArrayFetch['cityId'];
					//$productRowResult = new graffitiMessageInstance(NULL);
					//echo("cityId= [" . $productRowResult->cityId . "]\n");
					echo("cityId= [" . $productRowResult->GeoNameId . "]\n");
					$productRowResult->CityName = $resultsArrayFetch['CityName']; 
					$productRowResult->State = $resultsArrayFetch['State']; 
					//$productRowResult->StateAlternative = $resultsArrayFetch['StateAlternative']; 
					$productRowResult->CountryCode 	 = $resultsArrayFetch['CountryCode']; 
					$productRowResult->first_latitude 	 = $resultsArrayFetch['first_latitude']; 
					//echo("\"first_latitude\": \"" . $resultsArrayFetch["first_latitude"] . "\",");
					$productRowResult->first_longitude 	 = $resultsArrayFetch['first_longitude']; 
					$productRowResult->nearestBigCity	= $resultsArrayFetch['nearestBigCity'];
					$productRowResult->nearestBigCityId	= $resultsArrayFetch['nearestBigCityId'];
					$productRowResult->County		= $resultsArrayFetch['County'];
					//echo("\"first_longitude\": \"" . $resultsArrayFetch["first_longitude"] );
                            		$bFound = true;
                                        array_push($arrayResults,$productRowResult);
				}
...
/* this rev takes just the lat and long and returns the cityName, shortState, longState, and CountryCode and Country */
	...
	public function getCityIdUsingDivision(&$city_Id, $name, $countryName, $division_Id, $latitude, $longitude){
		$bFoundId = false;
		$url1 = "http://api.geonames.org/findNearbyPlaceName?lat=" . $latitude . "&lng=" . $longitude . "&style=FULL&username=pgits";
		error_log($url1 , 3, "/home1/produdn8/etc/groupon/NewCityLookup.usingDivision.log");
		$xml = file_get_contents($url1);
		error_log($xml , 3, "/home1/produdn8/etc/groupon/NewCityLookup.usingDivision.log");
		...
		$strSql = $this->sqlGetCityIdDivision .  " ( CityName = \"" . $CityName . "\" ) " . " AND " . " CountryCode = \"" . $CountryCode . "\" AND ( State = \"" . $StateProvinceShortName . "\"  ) ;"; 
		//$strSql = $this->sqlGetCityId .  "CityName = \"" . $CityName . "\" AND " . " CountryCode = \"" . $CountryCode . "\" AND ( State = \"" . $StateProvinceShortName . "\"  OR StateAlternate = \"" . $StateProvinceName . "\" OR State = \"" . $StateProvinceName. "\"  OR StateAlternate = \"" . $StateProvinceShortName . "\" ) ;"; 
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");
		error_log("strSql = [" . $strSql . "]\n" , 3, "/home1/produdn8/etc/groupon/NewCityLookup.usingDivision.log");
		...
		if($res){
			while($newArray = mysqli_fetch_array($res, MYSQLI_ASSOC)){
				$city_Id 	= $newArray['cityId'];
				$found_latitude = $newArray['first_latitude'];
				$found_longitude= $newArray['first_longitude'];
				//update division_Id --> County, lat, lng
				echo("swapping lat=[". $found_latitude . "],lng=[" . $found_longitude . "] with lat=[" . $latitude . "],lng=[" . $longitude . "]\n");
				$this->updateCityDivision( $city_Id, $division_Id, $CountryName, $latitude, $longitude, $CityName, $name);
				echo("<cityId>" . $city_Id . "</cityId>");
				error_log("<cityId>" . $city_Id . "</cityId>" , 3, "/home1/produdn8/etc/groupon/NewCityLookup.usingDivision.log");
				$bFoundId = true;
				return true;
			}
			if($bFoundId == false)
				$bFoundId = $this->addCityIdUsingDivision($city_Id, $CityName, $StateProvinceName, $StateProvinceShortName, $CountryName, $CountryCode, $latitude, $longitude, $GeoNameId, $division_Id, $name);
		}
		if($bFoundId == false) {
			echo("</cityId>");
			error_log("</cityId>\n" , 3, "/home1/produdn8/etc/groupon/NewCityLookup.usingDivision.log");
	...
	public function getCityId_2(&$city_Id, $latitude, $longitude){
		...
		$strSql = $this->sqlGetCityId .  " ( CityName = \"" . $CityName . "\" ) " . " AND " . " CountryCode = \"" . $CountryCode . "\" AND ( State = \"" . $StateProvinceShortName . "\"  ) ;"; 
		//$strSql = $this->sqlGetCityId .  "CityName = \"" . $CityName . "\" AND " . " CountryCode = \"" . $CountryCode . "\" AND ( State = \"" . $StateProvinceShortName . "\"  OR StateAlternate = \"" . $StateProvinceName . "\" OR State = \"" . $StateProvinceName. "\"  OR StateAlternate = \"" . $StateProvinceShortName . "\" ) ;"; 
		if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");
		error_log("strSql = [" . $strSql . "]\n" , 3, "/home1/produdn8/etc/groupon/NewCityLookup.2.log");
...
//giving them a new id using just their guid which could realistically change. -- this is distinguished in the code with a graffiti_author_Id but no author_alias value and will default to a female, age 21
	public function addAuthorIdCloaked( $adGUID, $latitude, $longitude){
		...
		error_log("calling from addAuthorIdCloaked" , 3, "/home1/produdn8/etc/groupon/NewCityLookup.2.log");
		$this->getCityId_2($city_Id, $latitude, $longitude);
		error_log("after getCityId_2 before city_Id = " . $city_Id . "\n" , 3, "/home1/produdn8/etc/groupon/NewCityLookup.2.log");
...
//Replace the alternates to the regular fields and copy the existing fields to the alternate, as long as the Alternates are not null.
	...
	public function updateCityDivision( $city_Id, $division_Id, $countryName, $lat, $lng, $CityNameUrl, $searchName){
		//$this->sqlUpdateCitiesStr = "UPDATE " .  "`" . $database . "`" .  "." .  $this->citiesTable . " SET `CityNameAlternate` = "; 
		//$strSql = $this->sqlUpdateCitiesStr . "\"" . $CityNameAlternate . "\", `StateAlternate` = \"" . $StateAlternate . "\" WHERE cityId = \"" . $city_Id . "\";"; 
		if($CityNameUrl === NULL)
			$strSql = $this->sqlUpdateCitiesDivisionStr . " SET `Country` = \"" . $countryName . "\", `County` = \"" . $division_Id . "\", `first_latitude` = \"" . $lat . "\", `first_longitude` = \"" . $lng . "\", `grouponSearchTerm` = \"" . $searchName . "\" WHERE cityId = \"" . $city_Id . "\";"; 
		else
			$strSql = $this->sqlUpdateCitiesDivisionStr . " SET `Country` = \"" . $countryName . "\", `County` = \"" . $division_Id . "\", `first_latitude` = \"" . $lat . "\", `first_longitude` = \"" . $lng . "\", `CityNameUrl` = \"" . $CityNameUrl . "\", `grouponSearchTerm` = \"" . $searchName . "\" WHERE cityId = \"" . $city_Id . "\";"; 
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("strSql = [" . $strSql . "]\n");
		$res = mysqli_query($this->mysqli, $strSql);
		if($res){
			echo("<updateCityDivision>SUCCESS</updateCityDivision>\n");
			return true;	
		}
		else {
			echo("<updateCityDivision>FAIL</updateCityDivision>\n");
			echo("strSql = [" . $strSql . "]\n");
		}
		return false;
	}
...
//populates the Swiggen with the details below, joining both coupon_details with pkpass_details
	...
	public function updateCities($strSqlUpdate){
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("updateCities = [" . $strSqlUpdate . "]\n");
		$res = mysqli_query($this->mysqli, $strSqlUpdate);
		if($res){
			if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
				printf("number of rows affected = %d", $this->mysqli->affected_rows);
			$numRowsUpdated = $this->mysqli->affected_rows;
		...
		echo("failed to update row = [" . $strSqlUpdate . "]\n");
		return $bInsert;
	}
	public function createCities($strSqlInsert){
		//if($GLOBALS['DEBUG_DATABASE_FLAG'] == TRUE)
			echo("createCities = [" . $strSqlInsert . "]\n");
		$res = mysqli_query($this->mysqli, $strSqlInsert);
		if($res){
			$bInsert = mysqli_insert_id($this->mysqli);
			echo("row inserted = [" . $bInsert . "]\n");
			return true;
		}
		echo("failed to insert row = [" . $strSqlInsert . "]\n");
		return $bInsert;
	}
	...
	public function value_in_debug($element_name, $xml, $content_only = true) {
    		if ($xml == false) {
        		return false;
    		}
    		$found = preg_match('#<' . $element_name. '(?:\s+[^>]+)?>(.*?)' .  '</' . $element_name . '>#s', $xml, $matches);
...
//HERE PETEY
	...
	public function __toString(){
		$outString = "<currency>\n";
	        $outString .= "\t <currency_Id>" .  		$this->currency_Id . 			"</currency_Id>\n";
	        $outString .= "\t <currency_code>" .  		$this->currency_code . 			"</currency_code>\n";
...
class couponAndRecipeInstance {
	...
	public function __toString(){
		...
		if($this->coupon_url_orig != NULL)
	        	$outString .= "\t <coupon_url_orig>". 	$this->coupon_url_orig  . 			"</coupon_url_orig>\n";
...
class citiesInstance {
	...
	public function __toString(){
		$outString = NULL;
	        ...
	        $outString .= "\t <CityNameUrl>" . $this->CityNameUrl . "</CityNameUrl>\n";
	        $outString .= "\t <State>" . $this->State . "</State>\n";
	        $outString .= "\t <CountryCode>" . $this->CountryCode . "</CountryCode>\n";
	        $outString .= "\t <first_latitude>" . $this->first_latitude . "</first_latitude>\n";
	        $outString .= "\t <first_longitude>" . $this->first_longitude . "</first_longitude>\n";
	        $outString .= "\t <population>" . $this->population . "</population>\n";
		return $outString;
	}
}
...
class couponDetailsInstance {
	...
	var $business_Id;
	var $coupon_url;
	var $coupon_url_orig;//this was added because the original href isn't the link we needed to massage in the Affiliate link
	var $swiggen_desc;
	var $percentage_under_60;
	var $percentage_under_120;
	var $percentage_over_120;
...
class graffitiMessageInstance6 {
  	var $city_Id;
  	var $graffiti_Id;
  	//var $country_code;
  	//var $created_datetime;
...
class storeDetailsInstance {
	...
	var $website_url;
	var $business_Id;
	var $groupon_store_Id;
	var $groupon_neighborhood;
	var $uuid;
...
class optOutInstance {
	...
	public function __toString(){
		$outString = NULL;
	        $outString .= "\t <graffiti_Id>" .  		$this->graffiti_Id . 			"</graffiti_Id>\n";
	        $outString .= "\t <transaction_Id>" .  		$this->transaction_Id . 			"</transaction_Id>\n";
	        $outString .= "\t <city_Id>" .  		$this->city_Id . 			"</city_Id>\n";
	        $outString .= "\t <viewer_Id>" .  		$this->viewer_Id . 			"</viewer_Id>\n";
	        $outString .= "\t <created>" .  		$this->created . 			"</created>\n";
	        $outString .= "\t <untilTime>" .  		$this->untilTime . 			"</untilTime>\n";
		return $outString;
	}
}
...
class geoCode {
	...
	var $cityResult;
        ...
        function GeoCode($CityName, $CountryCode){
                $this->url1 = "http://api.geonames.org/search?q=" . $CityName . "&country=" . $CountryCode . "&maxRows=1&cities=cities1000&isNameRequired=true&style=FULL&username=pgits";
                echo $this->url1 . "\n";
                $this->xml = file_get_contents($this->url1);
                echo "returned [" . $this->xml . "]\n";
...
class geoCodeInstance {
	...
	var $zipCode;
        ...
        var $xml;
	var $id;
	var $grouponSearchTerm;
	var $calculatedDistanceAway;
	var $currency;
	
        function reverseGeoCode($lat, $lon){
		$this->first_latitude        = $lat;
		$this->first_longitude       = $lon;
                $this->url1 = "http://api.geonames.org/findNearbyPlaceName?lat=" . $lat . "&lng=" . $lon . "&style=FULL&username=pgits";
                echo $this->url1 . "\n";
                $this->xml = file_get_contents($this->url1);
                echo "returned [" . $this->xml . "]\n";
        ...
        function GeoCode($CityName, $CountryCode){
                ...
                $this->xml = file_get_contents($this->url1);
                echo "returned [" . $this->xml . "]\n";
                $this->first_latitude               = value_in('lat', $this->xml, true);
                $this->first_longitude              = value_in('lng', $this->xml, true);
                $this->GeoNameId              = value_in('geonameId',$this->xml, true);
                $this->CityName               = value_in('name',$this->xml, true);
                $this->CountryCode            = value_in('countryCode', $this->xml, true);
                $this->CountryName            = value_in('countryName', $this->xml, true);
        ...
        public function __toString(){
                ...
                $outString .= "\t <CountryName>" .                            $this->CountryName .           "</CountryName>\n";
                $outString .= "\t <cityResult>" .                             $this->cityResult .           "</cityResult>\n";
                $outString .= "\t <County>" .                                 $this->County .           "</County>\n";
                $outString .= "\t <grouponSearchTerm>" .                      $this->grouponSearchTerm . "</grouponSearchTerm>\n";
                $outString .= "\t <calculatedDistanceAway>" .                 $this->calculatedDistanceAway . "</calculatedDistanceAway>\n";
                $outString .= "\t <zipCode>" .                 		      $this->zipCode . "</zipCode>\n";
		if($this->currency != NULL)
                	$outString .= "\t <currency>" .                       $this->currency . "</currency>\n";
		if($this->url1 != NULL)
                	$outString .= "\t <url1>" .                       $this->url1 . "</url1>\n";
                return $outString;
        }
}
