...
if ($conn->connect_error) {
	...
	$sqlString = "SELECT transactionId, mergeStatus, packageName, packageBuiltPath, packageRevision, lastGoodBuildUsed, OS_RELEASE, operation, buildRetries, smokeTestTriesBase, serverName, imageBaseName, imageBaseFullPathAndName, pullRequestId FROM $database.mergeTriggerPassedSmokeTest WHERE branch = \"$branch\" AND packageName = \"$packageName\" AND mergeStatus > 0 ;";
	if($DEBUG == TRUE)
	...
	if($result = mysqli_query($conn, $sqlString)){
		...
		if($rowcount == 0){
			...
			$continuumId = "";
			$packageRevision = "";
			$sqlString = "SELECT transactionId, pipeline_id, packageRevision  FROM $database.PackagesPassedSmokeTest WHERE branch = \"$branch\" AND packageName = \"$packageName\" AND operation = 12 LIMIT 1;";
        		if($DEBUG == TRUE)
                		echo "\nthis is what we are sending to mySql:  [" . $sqlString . "]\n";
			if($result = mysqli_query($conn, $sqlString)){ 
